...
<body>
	...
	try {
	    ...
	    String sql = "INSERT INTO time (carnum, intime) VALUES (?,now());";
	    stmt = MyConn.prepareStatement(sql);
	
        stmt.setString(1, rCarnum);
        stmt.executeUpdate();
        
        sql = "SELECT cellnum FROM reservation WHERE useridx = ?;";
	    stmt = MyConn.prepareStatement(sql);
        ...
        sql = "UPDATE pavailable SET carnum = ? WHERE cellnum = ?;";
	    stmt = MyConn.prepareStatement(sql);
	
        stmt.setString(1, rCarnum);
        stmt.setString(2, targetCellnum);
        stmt.executeUpdate();        
%>
<div id="confirmCard">
<h2>입차 확인되었습니다.</h2>
